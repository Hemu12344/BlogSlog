...
app.get("/removeCmt/:cmtId/:pId", async function (req, res) {
    const cmtId = req.params.cmtId;
    const pId = req.params.pId
    if (cmtId) {
        const comment = await postModel.findById(pId);
        const updatedComments = comment.comments.filter((val) => val._id.toString() !== cmtId);
